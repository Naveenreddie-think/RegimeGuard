...
## 9. Retraining Cadence & Drift Detection — Resolved

- **Recalibration cadence:** quarterly floor, set from real evidence rather than a round-number guess — a short-gap (3mo/6mo) stability follow-up found interior regime-label stability essentially solved at quarterly cadence (mean interior ARI ≈0.978 at 3 months), with the remaining instability confined to the edge zone during a live, ongoing stress event (not fixable by cadence at all). Full derivation: `docs/phase2_working_notes.md`, "short-gap follow-up" and "recalibration design" entries.
- **Drift detection:** a two-tier monitoring system — a cheap tier-1 standardization-drift metric (checked continuously) that only triggers an expensive tier-2 shadow-fit ARI confirmation when it fires, avoiding acting on tier-1 noise alone. Implemented in `regime_detection/monitoring.py`, with the tier-1 threshold itself grounded in real measured drift levels rather than picked arbitrarily. Full derivation: `docs/phase2_working_notes.md`, "two-tier monitoring" entries.
- **Versioned, point-in-time-honest infrastructure:** an append-only `model_versions`/`regime_labels` schema (`regime_detection/regime_db.py`) makes every past regime label reconstructable exactly as it would have been known at the time, not just as later hindsight revised it. Built out and stress-tested in Phase 3 via a 63-cutoff quarterly recalibration walk (`regime_detection/quarterly_walk.py`) that reproduces what a live system would actually have classified at each historical date — including an explicit, verified check (centroid-alignment across all 61 consecutive quarterly fits) that cross-fit regime-label identity holds throughout the evaluation-relevant period. Full derivation and results: `docs/phase3_working_notes.md`, "point-in-time regime-stratified re-run" entries.
- **Known remaining gap, not blocking:** the quarterly-walk infrastructure has no minimum-sample-size floor on its earliest possible cutoff (surfaced a degenerate fit at n=27 rows, confirmed harmless to every result produced so far but worth fixing before reuse at finer granularity, e.g. intraday) - logged as backlog in `docs/phase3_working_notes.md`.
